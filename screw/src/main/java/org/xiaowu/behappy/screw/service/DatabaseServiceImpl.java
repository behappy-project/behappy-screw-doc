...
import org.xiaowu.behappy.screw.util.AuthContextHolder;
...
@Service
@AllArgsConstructor
public class DatabaseServiceImpl extends ServiceImpl<DatabaseMapper, Database> implements IService<Database> {
    ...
    private final RoleDatabaseServiceImpl roleDatabaseServiceImpl;
    ...
    public void contextLoads(ScrewContextLoadDto screwContextLoadDto, String jdbcUrl, String driverClass) {
        if (!CollectionUtils.isEmpty(screwContextLoadDto.getIds())) {
            ...
            for (Database database : databases) {
                ...
                // 再查出数据源配置信息
                Datasource datasource = datasourceServiceImpl.getOne(new LambdaQueryWrapper<Datasource>()
    ...
    public void delete(Integer id) {
        ...
        if (databaseServiceImpl.removeBatchByIds(ids)) {
            // 删除role-database关联表数据
            roleDatabaseServiceImpl.removeByDatabaseId(ids);
        }
    }
}
