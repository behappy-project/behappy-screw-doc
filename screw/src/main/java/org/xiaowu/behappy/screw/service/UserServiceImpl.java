...
import org.xiaowu.behappy.screw.enums.RoleEnum;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IService<User> {
            ...
            if (authenticate(userDTO.getUsername(), userDTO.getPassword())) {
                ...
                if (!CollectionUtils.isEmpty(userList)) {
                    ...
                    if (userInfo == null) {
                        ...
                        userServiceImpl.saveOrUpdate(dbUser);
                        userServiceImpl.saveUser(dbUser);
                    } else {
                        dbUser = userInfo;
                    }
                }
    ...
    private void setDatabases(UserDto userDTO) {
        ...
        // 查出系统所有的菜单(树形)
        List<Database> databases = databaseServiceImpl.findDbs("");
        // new一个最后筛选完成之后的list
