  ...
  <div>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"
      ...
      <el-table-column label="操作"  width="300" align="center">
        <template slot-scope="scope">
          <el-button type="primary" @click="handleAdd(scope.row.id)" v-if="!scope.row.pid && !scope.row.path">新增数据库 <i class="el-icon-plus"></i></el-button>
...
export default {
  ...
  methods: {
    load() {
      ...
      // 请求图标的数据
      this.request.get("/database/icons").then(res => {
    ...
    delBatch() {
      ...
      this.request.post("/database/del/batch", ids).then(res => {
        if (res.code === '200') {
